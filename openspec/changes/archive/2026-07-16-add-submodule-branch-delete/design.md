...
After topology mutation, sync captures the resulting `.gitmodules` as the metadata-planning snapshot and refuses an atomic reconciliation if the live file no longer matches that snapshot. It captures the exact current working-tree `oms.yaml` bytes before alias mutation and stages those bytes verbatim rather than re-reading the path. Before advancing `HEAD`, finalization hashes the real index, acquires the OMS finalization lock, and rechecks that hash. Precise pre-/post-topology delta verification and full `HEAD`/manifest/`.gitmodules`/gitlink snapshot revalidation are intentionally out of scope; concurrent external working-tree edits during one sync may therefore require manual review, while single-actor workflows retain deterministic captured-input behavior.
