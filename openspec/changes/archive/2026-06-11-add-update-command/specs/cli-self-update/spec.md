...
The system SHALL update the current CLI installation automatically only when the installation is confidently classified as global and a newer npm registry version exists.

#### Scenario: Interactive global update
- **WHEN** the user runs `oms update`, a newer version exists, and a confident global update command is available
- **THEN** the command displays the current version, latest version, detected context, and package-manager command before asking for confirmation

#### Scenario: User confirms global update
- **WHEN** the user confirms the interactive update prompt
...
#### Scenario: Non-interactive confirmed global update
- **WHEN** the user runs `oms update --yes`, a newer version exists, and a confident global update command is available
- **THEN** the command runs the selected package-manager global update command without prompting
