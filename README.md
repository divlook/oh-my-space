...
| `oms commit [alias]` | workspace root or inside `oms/<alias>/` | Prepares the selected registration, then commits source changes inside the submodule only; never the root gitlink. | Initializes a registered alias automatically; refuses an unregistered alias. `-m <message>` is required (repeatable). Commits existing staged changes as-is, otherwise stages all with `git add -A`. |
...
| `oms branch switch [alias] [branch]` | workspace root | Prepares one declared source, then `git switch`es to or creates a LOCAL branch (no remote required). | Initializes an existing registration or offers sync for an unregistered alias. `--from <ref>` sets a new branch's start point. |
| `oms branch checkout [alias] [branch]` | workspace root | Prepares one declared source, fetches origin, then checks out a REMOTE branch (`origin/*`) as a local tracking branch. | Initializes an existing registration or offers sync for an unregistered alias. To create a brand-new local branch, use `oms branch switch`. |
| `oms branch list [alias]` | workspace root | Prepares one declared source, refreshes all declared remotes, and lists local and remote-tracking branches. | Initializes an existing registration or offers sync for an unregistered alias. Shows baseline certainty, divergence, detached HEAD, and per-remote freshness. |
| `oms branch delete [alias] [branch]` | workspace root | Prepares one declared source, then deletes one LOCAL branch with `git branch -d` or `-D` (`--force`). | Initializes an existing registration but refuses an unregistered alias. Never deletes a remote ref or changes root topology. Destructive targets are never auto-selected. |
| `oms fetch [alias...]` / `--all` | workspace root | Prepares each selected source, then runs `git fetch <remote> --prune`. | Initializes existing registrations; offers one batched sync for unregistered aliases, whose acceptance creates one root topology commit. `--remote <name>` is repeatable. |
| `oms pull [alias...]` / `--all` | workspace root | Prepares each selected source, then runs `git pull --ff-only <remote>` on its current branch. | Uses the same registration offer as fetch. Never records the root gitlink; rejects dirty source changes and prints an `oms record` hint when the pointer moves. |
| `oms push [alias...]` / `--all` | workspace root | Prepares each registered source, then runs `git push <remote> <branch>`. | Initializes existing registrations but refuses unregistered aliases. Never records the root gitlink. `--commit`/`--record` are unsupported; use `oms record <alias>`. |
