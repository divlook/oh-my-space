...
- **One finalization.** Reconciled metadata follows the same commit-or-unstage decision as topology, and that decision defaults to committing: one path-limited commit records topology and metadata together along with the complete current working-tree `oms.yaml`. The default is identical whether or not stdin is a terminal. Pass `--no-commit` to leave the changes unstaged instead; `--commit` is accepted for compatibility and matches the default.
- **Partial success and isolation.** A finalization that partially fails commits only the successful aliases through an owner-only temporary index, preserving unrelated staged paths, and exits non-zero with a summary. The temporary-index commit, a durable fsynced intent marker, and an atomically installed replacement index keep an interrupted commit recoverable: `sync`, `unsync`, and `record` run a shared recovery preflight that completes or safely blocks on leftover finalization state before mutating the root again.
...
- **Preserve the recorded checkout.** `oms sync` attaches the baseline branch only when it can do so without moving the checked-out commit. If the baseline has advanced past the recorded pointer, sync stays detached at the recorded commit and prints explicit branch-switch and pull guidance instead of acting as an implicit pull.
...
All commands that operate inside a submodule use the same preparation classifier. A registration that
exists in root `HEAD`, the index, and the working tree but is not initialized is initialized automatically
without changing root topology. Partial or inconsistent registration is always refused with `oms sync`
repair guidance. Declared but unregistered aliases follow this matrix:

| Command | Registered but uninitialized | Declared but unregistered |
| --- | --- | --- |
| `oms commit` | initialize and continue | refuse; a fresh clone has no source changes |
| `oms fetch` | initialize and continue | offer sync; acceptance creates one root topology commit |
| `oms pull` | initialize and continue | offer sync; acceptance creates one root topology commit |
| `oms push` | initialize and continue | refuse; a fresh clone has no unpushed commits |
| `oms branch list` | initialize and continue | offer sync; acceptance creates one root topology commit |
| `oms branch switch` | initialize and continue | offer sync; acceptance creates one root topology commit |
| `oms branch checkout` | initialize and continue | offer sync; acceptance creates one root topology commit |
| `oms branch delete` | initialize and continue | refuse; a fresh clone has no deletable local branch |

Registration is never created silently. Non-interactive offers fail without changing topology. A named
alias or sole candidate defaults to sync; `--all` and multi-select default to skipping unregistered aliases.
Multi-alias fetch and pull ask once and register accepted aliases in one topology commit. For commit, pull,
and push, a detached `HEAD` attaches automatically when a local branch points at the same commit; otherwise
OMS asks before moving the checkout or exits with `oms branch switch` guidance.
...
| `oms sync <alias>` / `--all` | workspace root | Registers missing repos with `git submodule add`, initializes registered-but-uninitialized ones, fetches, and attaches the baseline branch when attachment preserves the checked-out commit. | Reproduces the recorded pointer on a fresh clone and does not advance to a newer baseline tip; use `oms branch switch` and `oms pull` to advance deliberately. Topology changes (`.gitmodules`, `oms/<alias>`) are committed by default (`chore(oms): add ...`); pass `--no-commit` to leave them unstaged. |
...
| `oms commit [alias]` | workspace root or inside `oms/<alias>/` | Prepares the selected registration, then commits source changes inside the submodule only; never the root gitlink. | Initializes a registered alias automatically; refuses an unregistered alias. `-m <message>` is required (repeatable). Commits existing staged changes as-is, otherwise stages all with `git add -A`. Infers a configured alias from the current `oms/<alias>/` directory; an explicit alias wins. |
| `oms record [alias...]` / `--all` | workspace root or inside `oms/<alias>/` | Commits existing root gitlink pointer updates in one commit (`chore(oms): update <alias> submodule to <sha>`, or `chore(oms): update submodules` for several). | Root repo only, path-limited to the selected `oms/<alias>` paths; refuses staged changes outside the selection. An explicit alias wins over current-path inference. A named alias that cannot be recorded fails; with `--all` it is reported and skipped (exit 2) while the other moved pointers are still recorded. Not for adds/removals — use `oms sync`/`oms unsync`. |
| `oms branch switch [alias] [branch]` | workspace root | Prepares one declared source, then `git switch`es to or creates a LOCAL branch (no remote required). | Initializes an existing registration or offers sync for an unregistered alias. `--from <ref>` sets a new branch's start point. Omit alias/branch to pick interactively (or create a new branch). |
| `oms branch checkout [alias] [branch]` | workspace root | Prepares one declared source, fetches origin, then checks out a REMOTE branch (`origin/*`) as a local tracking branch (or switches to an existing local counterpart). | Initializes an existing registration or offers sync for an unregistered alias. Omit alias/branch to pick interactively. To create a brand-new local branch, use `oms branch switch`. |
| `oms branch list [alias]` | workspace root | Prepares one declared source, refreshes all declared remotes, and lists local and remote-tracking branches. | Initializes an existing registration or offers sync for an unregistered alias. Shows baseline certainty, divergence, detached HEAD, and per-remote freshness. Never changes a root gitlink outside an explicitly accepted `oms sync`; the only branch change is attaching a detached HEAD to a local branch at that commit, creating it there when it does not exist yet. |
| `oms branch delete [alias] [branch]` | workspace root | Prepares one declared source, then deletes one LOCAL branch with `git branch -d` or `-D` (`--force`). | Initializes an existing registration but refuses an unregistered alias. Never deletes a remote or remote-tracking ref, never touches the root gitlink. Protects the current branch and every resolved baseline (see below). Destructive targets are never auto-selected. Exit 0 on success, 1 on input/precondition errors, 2 on a declined/failed/concurrent deletion. |
| `oms fetch [alias...]` / `--all` | workspace root | Prepares each selected source, then runs `git fetch <remote> --prune`. | Initializes existing registrations; offers one batched sync for unregistered aliases, whose acceptance creates one root topology commit. `--remote <name>` (repeatable) picks the remote(s); omit to choose interactively, defaults to `origin`. |
| `oms pull [alias...]` / `--all` | workspace root | Prepares each selected source, then runs `git pull --ff-only <remote>` on its current branch. | Uses the same registration offer as fetch. Never records the root gitlink; rejects dirty source changes and prints an `oms record <alias>` hint when the pointer moves. `--remote <name>` selects a single remote (defaults to `origin`). |
| `oms push [alias...]` / `--all` | workspace root | Prepares each registered source, then runs `git push <remote> <branch>`. | Initializes existing registrations but refuses unregistered aliases. Never records the root gitlink. Omit the aliases to pick interactively. `--commit`/`--record` are unsupported; use `oms record <alias>`. `--remote <name>` (repeatable) picks the remote(s); upstream is set only for `origin`. |
| `oms unsync <alias>` / `--all` | workspace root | `git submodule deinit` + `git rm` for the alias; drops an empty `.gitmodules`. | Keeps the `oms.yaml` entry. Use `--force` to discard uncommitted changes. Removal topology is committed by default (`chore(oms): remove ...`); pass `--no-commit` to leave it unstaged. |
